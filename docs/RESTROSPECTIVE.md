...
## 트러블 슈팅 17 - BPE Tokenizer 개선 시도 및 3단계 정리 결정

### 문제 상황

3단계에서 한국어 데이터를 사용하여 BPE Tokenizer를 학습한 결과, 생성 품질이 기대만큼 나오지 않았다. Loss는 어느 정도 감소했으나, 실제 생성 결과에서는 반복 현상이 심하고 문법적으로 올바른 문장이 거의 나타나지 않았다.

이에 따라 BPE Tokenizer의 학습 방식(특히 merges_needed 계산 방식)을 개선하여 vocabulary 품질을 높여보고자 시도했다.

### 결정 및 대응

**1. merges_needed 계산 방식 변경**

기존에는 self.vocab_size - len(initial_tokens) 방식으로 병합 횟수를 계산하고 있었으나, 이 방식이 BPE 학습의 안정성과 vocabulary 품질에 부정적인 영향을 줄 수 있다고 판단했다.

따라서 아래와 같이 계산 방식을 변경했다:
...
이 변경을 통해 Loss가 더 낮게 수렴하는 결과를 얻을 수 있었으나(0.54 → 0.076), 생성 품질 자체는 크게 개선되지 않았다.

**2. 3단계 최소 정리 결정**

BPE Tokenizer를 추가로 개선하는 데 드는 시간 대비 효과가 크지 않다고 판단했다. 특히 현재 데이터 규모와 모델 구조에서는 한국어 생성 품질을 의미 있게 끌어올리기 어려울 것으로 보였다.

따라서 **3단계는 최소한으로 정리**하고 4단계로 넘어가기로 결정했다. BPE Tokenizer 코드의 불필요한 주석은 정리하고, 핵심 로직만 남기는 방향으로 정리했다.

### 인사이트

- BPE의 merges_needed 계산 방식은 단순히 목표 vocabulary 크기만 고려하는 것이 아니라, 학습의 안정성과 subword 품질을 함께 고려해야 한다는 점을 확인했다.
- 한국어를 from-scratch로 다루는 경우, 영어에 비해 데이터 규모와 전처리 방식이 훨씬 더 중요하다는 것을 다시 한 번 체감했다.
- 제한된 시간과 리소스 내에서 모든 단계를 완벽하게 수행하기보다는, **효과가 제한적인 부분은 과감하게 정리**하는 판단도 중요하다는 것을 배웠다.
- 3단계에서 기대한 만큼의 결과를 내지 못했지만, BPE 학습 방식의 민감성과 한국어 토큰화의 어려움을 직접 경험한 것은 의미 있는 과정이었다.
